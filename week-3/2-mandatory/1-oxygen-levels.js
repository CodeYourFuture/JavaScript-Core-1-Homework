...
function safeLevels(levels) {
  function getNumber(precentage) {
    const onlyDigits = precentage.slice(0, precentage.length - 1);
    const number = Number(onlyDigits);
    return number;
  }

  function checkSafeOxygen(level) {
    const minimumOxygen = 19.5;
    const maximumOxygen = 23.5;

    return level > minimumOxygen && level < maximumOxygen;
  }

  const numberResult = levels.map(getNumber).find(checkSafeOxygen);
